...
class Local3dSolver(BaselineSolver):
    ...
    def train(self):
        ...
        for seq, view, seq_type, label in self.trainloader:
            ...
            t_reg = (torch.norm(deltas[3] - 0.4).div(num) + \
                     torch.norm(deltas[4] - 0.4).div(num) + \
                     torch.norm(deltas[5] - 0.4).div(num)) / 3
    ...
    def _test(self):
        ...
        if len(acc_compact) > 1:
            self.writer.add_scalar('test_full/AccNM', acc_full[0], self.iter)
            self.writer.add_scalar('test_full/AccBG', acc_full[1], self.iter)
            self.writer.add_scalar('test_full/AccCL', acc_full[2], self.iter)
            self.writer.add_scalar('test_local/AccNM', acc_local[0], self.iter)
            self.writer.add_scalar('test_local/AccBG', acc_local[1], self.iter)
            self.writer.add_scalar('test_local/AccCL', acc_local[2], self.iter)
            self.writer.add_scalar('test_compact/AccNM', acc_compact[0], self.iter)
            self.writer.add_scalar('test_compact/AccBG', acc_compact[1], self.iter)
            self.writer.add_scalar('test_compact/AccCL', acc_compact[2], self.iter)
        else:
            self.writer.add_scalar('test/AccFull', acc_full[0], self.iter)
            self.writer.add_scalar('test/AccLocal', acc_local[0], self.iter)
            self.writer.add_scalar('test/AccCompact', acc_compact[0], self.iter)
        target_acc = getattr(self.cfg, 'target_acc', 'full')
        accs = {'full': acc_full, 'local': acc_local, 'compact': acc_compact}
    ...
    def vis_fusion_weight(self):
        ...
        if 'feature_fusion' in self.model.module.local and vis_fusion_weight:
            ...
            global_channels = y.shape[0] - 3*local_channels
            ...
            sns.lineplot(x=x[x2:x3], y=y[x2:x3], label='torso')
            sns.lineplot(x=x[x3:], y=y[x3:], label='legs')
