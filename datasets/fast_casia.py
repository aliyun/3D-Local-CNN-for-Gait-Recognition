...
def _CASIA_OUMVLP(batch_size=[8, 16], test_batch_size=1, num_workers=3,
    ...
    """
    train_list_path = osp.join(list_path, '{}_{}_train_list_seq.npy'.format(
        pid_num, pid_shuffle))
    test_list_path = osp.join(list_path, '{}_{}_test_list_seq.npy'.format(
        pid_num, pid_shuffle))
    ...
    train_index_path = osp.join(list_path, '{}_{}_train_index_seq.npy'.format(
        pid_num, pid_shuffle))
    test_index_path = osp.join(list_path, '{}_{}_test_index_seq.npy'.format(
        pid_num, pid_shuffle))
...
class DataSet(tordata.Dataset):
    def __init__(self, seq_dir, label, seq_type, view, resolution,
                 frame_num=-1, index_dict=None):
        r"""
        Attributes:
    ...
    def img2xarray(self, file_path):
        ...
        if frame_num > 0:
            ...
            intercept_id_list = [frame_set[i] for i in range(start, start+intercept_length)]
            intercept_index = sorted(np.random.permutation(
                list(range(intercept_length)))[:frame_num])
            frame_id_list = [intercept_id_list[i] for i in intercept_index]
        else:
            frame_id_list = frame_set

        frame_list = [np.reshape(
            cv2.imread(osp.join(file_path, imgs[i])),
            [self.resolution, self.resolution, -1])[:, :, 0]
                      for i in frame_id_list
                      if osp.isfile(osp.join(file_path, imgs[i]))]
