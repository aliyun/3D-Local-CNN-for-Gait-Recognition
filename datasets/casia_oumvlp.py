...
def _CASIA_OUMVLP(batch_size=[8, 16],
    ...
    train_list_path = osp.join(
        list_path, '{}_{}_train_list.npy'.format(pid_num, pid_shuffle))
    test_list_path = osp.join(
        list_path, '{}_{}_test_list.npy'.format(pid_num, pid_shuffle))
    ...
    train_index_path = osp.join(
        list_path, '{}_{}_train_index.npy'.format(pid_num, pid_shuffle))
    test_index_path = osp.join(
        list_path, '{}_{}_test_index.npy'.format(pid_num, pid_shuffle))
...
class DataSet(tordata.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if not self.cache:
            ...
            frame_set = [
                set(feature.coords['frame'].values.tolist())
                for feature in data
            ]
    ...
    def img2xarray(self, flie_path):
        ...
        frame_list = [
            np.reshape(cv2.imread(osp.join(flie_path, _img_path)),
                       [self.resolution, self.resolution, -1])[:, :, 0]
            for _img_path in imgs if osp.isfile(osp.join(flie_path, _img_path))
        ]
