...
def collate_fn(batch, frame_num, sample_type):
    ...
    def select_frame(index):
        ...
        if sample_type == 'random':
            ...
            sampled_seq = [feature.loc[frame_id_list].values for feature in sample]
    ...
    if sample_type == 'random' or sample_type == 'sequential':
        ...
        batch_frames = [[len(frame_sets[i])
                         for i in range(batch_per_gpu * j, batch_per_gpu * (j + 1))
                         if i < batch_size ] for j in range(gpu_num)]
        ...
        max_sum_frame = np.max([np.sum(batch_frames[i]) for i in range(gpu_num)])
        # concat frames of all sequences from one gpu
        # seqs: [feature_num x gpu_num]
        seqs = [[np.concatenate([seqs[i][j]
                                 for i in range(batch_per_gpu * k, batch_per_gpu * (k + 1))
                                 if i < batch_size], 0) for k in range(gpu_num)]
        ...
        seqs = [np.asarray([np.pad(seqs[j][i],
                                   ((0, max_sum_frame - seqs[j][i].shape[0]), (0, 0), (0, 0)),
                                   'constant', constant_values=0) for i in range(gpu_num)])
                for j in range(feature_num)]
